...
public class KmxxzcToExcel extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// System.out.println("Execute jdbcSql in Servlet : "+jdbcSql);
		int length = Integer.parseInt(request.getParameter("length"));
		// System.out.println("-----------Export columns is :
		// -------------"+length);
		System.out.println("ExcelName:" + excelName + " Title:" + title
				+ " \nJdbcSql:" + jdbcSql);
		...
		Label label;
		try {
			sheet.mergeCells(0, 0, length - 1, 1);// 合并单元格(左列，左行，右列，右行)从第1行第1列到第2行第3列
			...
			// label = new Label(5, 5, "Hello world");
			// sheet.addCell(label);
		...
		// System.out.println("List:"+list+" OutputStream : "+out+" workbook :
		// "+workbook);
		try {
			workbook.write();
			workbook.close();
